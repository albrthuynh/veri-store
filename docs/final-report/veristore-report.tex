...
\textbf{Design choice}: A Cauchy matrix over $\mathrm{GF}(256)$ is used (implemented in \texttt{src/erasure/matrix.py}) rather than a Vandermonde matrix, because Cauchy matrices are always invertible for any choice of distinct field elements, simplifying the proof of correctness and eliminating degenerate parameterizations.

\subsection{Homomorphic Fingerprinting and the \texttt{fpcc}}

For a fragment $d_i$ of length $L$ bytes, the fingerprint at evaluation point $r \in \mathrm{GF}(256)$ is: $$ \mathrm{fp}(r, d_i) = \sum_{j=0}^{L-1} d_i[j] \cdot r^j, \quad r, d_i[j] \in \mathrm{GF}(256) $$

This is a polynomial evaluation over $\mathrm{GF}(256)$ and is homomorphic: for erasure-coded fragments, $\mathrm{fp}(r, d_0)$ encodes information about all other fragments, so consistency across fragments can be checked without reconstructing the full block.

The fingerprinted cross-checksum (\texttt{fpcc}) is generated at upload time and distributed to all servers alongside the fragment and consists of:
\begin{itemize}
...
\textbf{Design choice}: The challenge point $r$ is derived from all fragment hashes, not from a fixed parameter. This means $r$ changes if any fragment changes, binding all fragments together cryptographically without requiring a trusted third party.
...
\begin{tikzpicture}[node distance=1.8cm]
  ...
  \node (fingerprint) [process, right of=encode, xshift=2.2cm]                        {Compute\\\texttt{fpcc}};
  \node (distribute)  [process, below of=fingerprint, yshift=-0.8cm]                  {Distribute\\Fragments};
  \node (network)     [entity, fill=untrusted!30, right of=distribute, xshift=2.2cm]  {Network};
  \node (storage)     [datastore, fill=semitrusted!30, below of=network, yshift=0.1cm] {Fragment\\Storage};
  \node (verify)      [process, below of=distribute, yshift=-0.8cm]                   {Verify\\Fragments};
  \node (reconstruct) [process, left of=verify, xshift=-2.2cm]                        {Reconstruct\\Block};

  \draw[arrow] (client)      -- node[right,  font=\scriptsize] {1.~data}              (encode);
  \draw[arrow] (encode)      -- node[above, font=\scriptsize] {2.~frags}             (fingerprint);
  \draw[arrow] (fingerprint) -- node[right, font=\scriptsize] {3.~\texttt{fpcc}}     (distribute);
  \draw[arrow] (distribute)  -- node[above, font=\scriptsize] {4.~\texttt{PUT}}      (network);
  \draw[arrow] (network)     -- node[right, font=\scriptsize] {5.~store}             (storage);
  \draw[arrow] (storage)     -- node[above, font=\scriptsize, xshift=-0.15cm, yshift=0.15cm] {6a.~retrieve} (verify);
  \draw[arrow] (verify)      -- node[above, font=\scriptsize] {7.~verified}          (reconstruct);
  \draw[arrow] (reconstruct) to[bend left=30]
    node[left, font=\scriptsize] {8.~block}                                           (client);
  \draw[arrow,dashed] (storage) to[bend left=40]
    node[below, font=\scriptsize, fill=white, inner sep=1pt] {6b.~cross-check}       (verify);
\end{tikzpicture}
...
\begin{description}[font=\normalfont, leftmargin=8.1em, labelwidth=7.6em, labelsep=0.5em, topsep=0pt, partopsep=0pt, parsep=0pt, itemsep=1em]
  \item[\textbf{1. data}] The client passes the raw data block to the encoder.
  \item[\textbf{2. frags}] The encoder produces $n$ erasure-coded fragments, passed to fingerprint computation.
  \item[\textbf{3. \texttt{fpcc}}] The \texttt{fpcc} (per-fragment hashes and GF(256) fingerprints) is computed across all $n$ fragments.
  \item[\textbf{4. \texttt{PUT}}] Each fragment and the shared \texttt{fpcc} are sent to their designated server.
  \item[\textbf{5. store}] Fragment Storage receives and persists each fragment and \texttt{fpcc} to disk.
  \item[\textbf{6a. retrieve}] On retrieval (\texttt{GET}), the server returns the stored fragment and \texttt{fpcc} together in a single response.
  \item[\textbf{6b. cross-check}] The \texttt{fpcc} from the same response serves as the integrity reference; the verifier uses it to check each fragment before reconstruction.
  \item[\textbf{7. verified}] Fragments that pass verification are forwarded to the reconstructor.
  \item[\textbf{8. block}] The reconstructed original data block is returned to the client.
\end{description}
...
{\renewcommand{\arraystretch}{1.5}
      ...
      \small
            \begin{tabular}{lll}
                  \hline
                  \textbf{Component} & \textbf{Library} & \textbf{Purpose} \\\hline
                  Finite field arithmetic & \texttt{src/fingerprint/field.py} & GF(256) polynomial ops \\\hline
                  Erasure coding & \texttt{src/erasure/} & Cauchy-matrix RS encode/decode \\\hline
                  Fragment verification & \texttt{src/verification/} & \texttt{fpcc} generation; fragment integrity checks \\\hline
                  Fragment persistence & \texttt{src/storage/} & On-disk fragment and metadata storage \\\hline
                  REST API server & \texttt{fastapi} $\geq$ 0.110, \texttt{uvicorn} & Fragment storage endpoints \\\hline
                  HTTP client & \texttt{httpx} $\geq$ 0.27 & Async parallel fragment dispersal \\\hline
                  API schema validation & \texttt{pydantic} $\geq$ 2.6 & Request/response schema enforcement \\\hline
...
\vspace{0.3em}
\textbf{Design choice}: \texttt{fastapi} and \texttt{pydantic} were chosen because schema validation happens automatically at the API boundary, rejecting malformed requests (e.g., impossible parameters like $m > n$) before they reach storage logic.
...
\subsection{Key Protocol: Dispersal (\texttt{PUT})}

\begin{enumerate}
  \item \texttt{encoder.py} pads the input to a multiple of $m$ bytes, constructs the Cauchy matrix, and produces $n$ fragments as \text{bytes} objects.
  \item \texttt{cross\_checksum.py} computes $\{h_i\}$, derives $r$ via \texttt{oracle.py}, and computes $\{\mathrm{fp}_i\}_{i<m}$ to produce the \texttt{fpcc}.
  \item \texttt{client.py} fans out $n$ parallel \texttt{httpx} \texttt{PUT} requests, each carrying the fragment (base64-encoded) and the serialized fpcc JSON.  A maximum of 3 retries with 0.5s linear backoff handles transient failures.
  \item The server-side \texttt{server.py} immediately calls \texttt{Verifier.check()} on the received fragment before persisting it to disk.
\end{enumerate}

\subsection{Key Protocol: Retrieval (\texttt{GET})}
\begin{enumerate}
      \item \texttt{client.py} issues $n$ parallel \texttt{GET} requests and collects the first $m$ successful responses using \texttt{Thread\-Pool\-Executor}.
      \item For each returned fragment, \texttt{Verifier.check()} runs two checks:
            \begin{enumerate}[label=(\alph*), topsep=0pt, itemsep=0pt, parsep=0pt]
                  \item \texttt{SHA-256} hash against the fpcc entry, and
                  \item for $i < m$, a fingerprint check: $\mathrm{fp}(r', d_i) \stackrel{?}{=} \texttt{fpcc.fingerprints}[i]$.
            \end{enumerate}
      \item Only fragments that return \texttt{CONSISTENT} are passed to \texttt{decoder.py}, which inverts the Cauchy matrix to recover the original block and strips padding.
\end{enumerate}
...
\textbf{Storage Server}: \emph{Semi-trusted} — may be compromised, return stale or corrupted fragments, or attempt to forge data.
...
\textbf{Theorem 3.4} (Hendrics et al.) bounds the miss probability of the fingerprint check at $\leq 1/|\mathrm{GF}(256)| = 1/256$ per check.  Empirically, testing all single-byte flips across 100 blocks ($\times$5 fragments each) yielded a $100\%$ detection rate and a $0.0\%$ false positive rate (\texttt{docs/experiments/theorem\_3\_4.md}).  The system tolerates up to $n - m = 2$ Byzantine faults: as long as $\geq 3$ honest servers are available, the client recovers the original block correctly.
...
\begin{itemize}
  ...
  \item \textbf{Endpoint tests}: malformed base64, impossible parameters
        ($m > n$), duplicate \texttt{PUT}, and missing bearer token all return the correct
